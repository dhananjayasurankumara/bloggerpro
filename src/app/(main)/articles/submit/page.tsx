...
export default function SubmitArticlePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await axios.post("/api/user/articles", formData);
    ...
    <div className="min-h-screen bg-white dark:bg-black pb-24">
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-sm md:text-base font-display font-bold whitespace-nowrap">Submission Lab</h1>
                <p className="text-[9px] text-gray-500 uppercase tracking-widest font-black">Editor Focused</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              disabled={loading}
              className="flex items-center gap-2 px-4 md:px-8 py-2.5 bg-primary text-white text-xs md:text-sm font-bold rounded-xl shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
              <span className="hidden sm:inline">Post to Review</span>
      ...
      <main className="max-w-4xl mx-auto px-6 pt-32">
        <div className="space-y-12">
          {/* Guiding Banner */}
          <div className="p-8 bg-zinc-950 rounded-[40px] border border-zinc-900 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-64 h-64 bg-primary opacity-5 blur-3xl -translate-y-32 translate-x-32 group-hover:opacity-10 transition-opacity"></div>
            <div className="relative z-10 flex items-start gap-6">
              <div className="p-4 bg-primary/20 rounded-2xl text-primary">
                <Sparkles className="w-6 h-6" />
              </div>
              <div className="space-y-2">
                <h2 className="text-2xl font-display font-bold text-white tracking-tight">Write for the 1%</h2>
                <p className="text-zinc-400 text-sm leading-relaxed max-w-lg">
                  Share your financial wisdom with the community. Once submitted, our expert editorial team will review your content for publication on the main feed.
                </p>
              </div>
            </div>
          </div>
          ...
          <form className="space-y-8" onSubmit={handleSubmit}>
            ...
            {/* Excerpt Section */}
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-500">Short Summary</label>
              <textarea 
                placeholder="Hook your readers with a 2-sentence summary..."
                className="w-full p-6 bg-gray-50 dark:bg-zinc-950 border border-gray-100 dark:border-zinc-900 rounded-3xl text-sm font-medium outline-none focus:ring-2 focus:ring-primary transition-all resize-none h-24"
            ...
            {/* Content Editor */}
            <div className="space-y-4">
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-500">The Content</label>
              <RichTextEditor 
                content={formData.content} 
                onChange={(html) => setFormData({ ...formData, content: html })}
                placeholder="Unleash your financial insights here..."
              />
            </div>
            ...
            <div className="pt-12 border-t border-gray-100 dark:border-zinc-900 flex flex-col md:flex-row justify-between items-center gap-8">
               <div className="flex items-center gap-4 p-4 bg-emerald-50 dark:bg-emerald-900/10 rounded-2xl max-w-sm">
                  <ShieldCheck className="w-10 h-10 text-emerald-500 shrink-0" />
                  <p className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 leading-relaxed uppercase tracking-wider">
                    Community Safety: All content is screened for quality and community guidelines before going live.
                  </p>
               </div>
               
               <button
                type="submit"
                className="w-full md:w-auto px-12 py-5 bg-black dark:bg-white text-white dark:text-black font-black rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-xl shadow-black/10 dark:shadow-white/5 uppercase tracking-widest text-xs"
              >
                Finalize & Submit
              </button>
            </div>
